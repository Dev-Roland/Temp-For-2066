...
--[[ Initialize Auto-WS Variables
local awsActive = true
local awsTpLimit = 1000
local awsWsName = "Vorpal Blade"
local awsFailsafeAttempts = 0
local awsFailsafeAttemptMax = 20]]
...
function get_sets()
	...
	-- Load AWS Plugin
	windower.send_command('lua load AutoWS;wait 1;aws ws Spirits Within;aws tp 3000;aws hp 20')
end
...
function init_gear_sets()
    ...
    -- Normal melee group
    sets.engaged = {
		head="Espial Cap", neck="Mirage Stole +1", left_ear="Tor. Earring +1", right_ear="Tor. Earring +1",
		body="Espial Gambison",hands="Espial Bracers", left_ring="Chirich Ring +1", right_ring="Chirich Ring +1",
		back="Aptitude Mantle +1", waist="Sweordfaetels", legs="Espial Hose", feet="Espial Socks",
    }
...
-- Register custom commands (call ingame via "//gs c commandNameHere")
function job_self_command(commandArgs, eventArgs)
	-- Set command
	local command = table.remove(commandArgs, 1) -- returns the deleted element
	local argument = table.concat(commandArgs, " ")
end
